...
class ConvNetb(nn.Module):
    ...
    def __init__(self, num_classes=60):
        ...
        self.fully_conv = nn.Conv2d(
            n * 16, num_classes, kernel_size=4, stride=1, padding=0, bias=True
        )  # 5 layer s1
...
def main(model):
    ...
    def train(model):
        ...
        for batch in range(int(nS / batch_size)):
            ...
            i = np.random.choice(
                nS, size=batch_size, p=weights
            )  # weighted chip selection
            x, y = X[i], Y[i]
    ...
    def test(model):
        ...
        for batch in range(int(nS / batch_size)):
            i = v[
                batch * batch_size : np.minimum((batch + 1) * batch_size, nS)
            ]  # ordered chip selection
...
def random_affine(
    ...
    R[:2] = cv2.getRotationMatrix2D(
        angle=a, center=(shape[1] / 2, shape[0] / 2), scale=s
    )

    # Translation
    T = np.eye(3)
    T[0, 2] = (
        (random.random() * 2 - 1) * translate[0] * shape[0]
    )  # x translation (pixels)
    T[1, 2] = (
        (random.random() * 2 - 1) * translate[1] * shape[1]
